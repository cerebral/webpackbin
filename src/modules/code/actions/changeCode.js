...
function changeCode ({props, state}) {
  const selectedFileIndex = props.codeChange.fileIndex || state.get('app.currentBin.selectedFileIndex')
  const code = state.get(`app.currentBin.files.${selectedFileIndex}.content`)

  state.set(`app.currentBin.files.${selectedFileIndex}.content`, transformCode(code, props.codeChange))
