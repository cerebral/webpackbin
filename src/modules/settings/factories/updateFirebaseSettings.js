  ...
  return sequence('updateFirebaseSettings', [
    function updateFirebaseSettings ({firebase, path, state}) {
      ...
      if (Array.isArray(keysOrKey)) {
        return firebase.update(`settings.${userUid}`, keysOrKey.reduce((payload, settingsPropKey) => {
          payload[settingsPropKey] = state.get(`settings.${settingsPropKey}`)
      ...
      return firebase.set(`settings.${userUid}.${keysOrKey}`, state.get(`settings.${keysOrKey}`))
        .then(path.success)
        .catch(path.error)
    }, paths ? Object.assign({
      success: [],
