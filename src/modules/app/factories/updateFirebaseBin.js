...
function updateFirebaseBinFactory(keysOrKey, paths) {
  return sequence('updateFirebaseBin', [
    function updateFirebaseBin({ firebase, path, state }) {
      ...
      const currentParticipantKey = state.get(
        'app.currentBin.currentParticipantKey'
      );
      ...
      if (Array.isArray(keysOrKey)) {
        return firebase
          .update(
            `bins.${binKey}`,
            keysOrKey.reduce((payload, binPropKey) => {
      ...
      return firebase
        .set(
          `bins.${binKey}.${keysOrKey}`,
          state.get(`app.currentBin.${keysOrKey}`)
        )
        .then(path.success)
        .catch(path.error);
    },
