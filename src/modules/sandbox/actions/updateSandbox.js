...
function updateSandbox({ http, state, path, resolve }) {
  ...
  const sortedPackages = Object.keys(currentBin.packages || {})
    .sort()
    .reduce((currentSortedPackages, packageKey) => {
      currentSortedPackages[decodeKey(packageKey)] =
        currentBin.packages[packageKey];
  ...
  return new Promise((resolve, reject) => {
    function makeRequest() {
      http
        .post(
          config.sandboxServiceUrl[region],
          {
        ...
        .catch(response => {
          if (
            response.status === 504 &&
            Date.now() - started < config.sandboxTotalTimeout
          ) {
            makeRequest();
          } else {
            reject(response);
